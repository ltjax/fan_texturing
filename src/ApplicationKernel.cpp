...
CApplicationKernel::CApplicationKernel(glsk::window& Wnd,const boost::filesystem::path& Filename)
									...
									std::vector<uint>& Indices )
{
	...
	{
		glsk::error_box( "Unable to open requested mesh file: \"" + Filename.string() + '\"' );
...
void CApplicationKernel::OnMouse( const glsk::mouse_event& Msg )
{
	switch ( Msg.get_type() )
	{
	case glsk::mouse_button:
		if ( Msg.get_index() != 1 )
			Camera.OnMouseButton( Msg.get_px(), Msg.get_py(), Msg.get_index(), Msg.get_value() );
		else
			DoDraw = Msg.get_value();
		break;

	case glsk::mouse_move:
		Camera.OnMouseMove( Msg.get_px(), Msg.get_py() );
		break;
	};

	MouseX = float(Msg.get_px());
	MouseY = float(Msg.get_py());
...
{
	HWND			WindowHandle = reinterpret_cast<HWND>(glsk::platform_win32::get_native_handle(Window));
